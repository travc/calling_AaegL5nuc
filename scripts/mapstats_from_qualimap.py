    ...
    with open(filename) as fh:
        for line in fh:
            ...
            if l[0].startswith('number of reads'):
                ...
                out['WG_cov_mean'] = float(l[1].replace('X','').replace(',','').strip())
            elif l[0].startswith('std coverageData'):
                out['WG_cov_std'] = float(l[1].replace('X','').replace(',','').strip())
            ## regions
            else:
                for k in contig_list:
                    if l[0].startswith(k+'\t'):
                        l2 = l[0].split('\t')
                        out[k+'_cov_mean'] = float(l2[3])
...
#    print >>sys.stderr, "START"
    ...
    for sample_dir in sorted(sample_dirs):
        ...
        if not header_done:
            header_done = True
            print( '#', time.strftime('%Y-%m-%dT%H:%M:%S%z'), file=sys.stdout)
        ...
        print(  d['name'],
                ...
                d['WG_cov_mean'],
                d['WG_cov_std'],
                d['total_reads']/d['WG_cov_mean'] if d['WG_cov_mean']>0 else 'nan',
                sep='\t',
                end='\t',
                file=sys.stdout)
        for region in contig_list:
